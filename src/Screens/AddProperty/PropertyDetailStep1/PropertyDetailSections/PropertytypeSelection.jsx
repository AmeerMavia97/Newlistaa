...
const PropertytypeSelection = ({
  ...
  useEffect(() => {
    ...
    if (multipleLeaseRates) {
      if (!isNaN(min) && !isNaN(max)) {
        setValue("leaseRate", `${min.toFixed(2)} - ${max.toFixed(2)}`);
      } else {
        setValue("leaseRate", "");
      }
    } else {
      if (!isNaN(min)) {
        setValue("leaseRate", min.toFixed(2));
      } else {
        setValue("leaseRate", "");
      }
    }
  ...
  useEffect(() => {
    console.log("leaseMinRate", leaseRateMin);
    console.log("leaseRateMax", leaseRateMax);
    console.log("multiple", multipleLeaseRates);
    console.log("leaseRate (final):", getValues("leaseRate"));
  }, [leaseRateMin, leaseRateMax, multipleLeaseRates]);
