  ...
  const handleFinalSubmit = async (data) => {
    ...
    if (status !== "active" && finalData.FeaturedListing === true) {
      if (cardFormRef.current) {
        ...
        // ✅ Always explicitly add paymentMethodId
        finalData.paymentMethodId = paymentResult.paymentMethodId;
      }
    } else {
      // ✅ Still add paymentMethodId with null if not featured
      ...
      <Elements stripe={stripePromise}>
        <form onSubmit={handleSubmit(handleFinalSubmit)}>
          ...
          {formData.fileInput?.length > 1 && (
            ...
            {status !== "active" && formData.FeaturedListing === true && (
              <StripeCardForm
                ref={cardFormRef}
            ...
            <div className="bg-[#f5f5f5] px-5 py-5 rounded-[8px]">
              ...
              <div className="mt-3 flex flex-col gap-1">
                ...
                <Controller
                  ...
                  control={control}
                  rules={{
                    required: "You must confirm authorization to list.",
                  }}
                  render={({ field }) => (
                    <Checkboxs
                      {...field}
